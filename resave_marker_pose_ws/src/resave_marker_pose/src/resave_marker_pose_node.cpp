...
class Resave_Marker_Pose{
	...
	Resave_Marker_Pose(std::string& to_save_path_,ros::NodeHandle n):to_save_path(to_save_path_),nh(n)
	{
		markerVisible = false;
		start_move = false;
		enoughMarker = false;

		sub_marker_msg_ = nh.subscribe("/aruco_single/aruco_marker", 30,&Resave_Marker_Pose::getMarkerMsgCallback,this);
	...
	bool selectMarkerPose(std::vector<marker_pose>& src_,\
			      ...
			      int n_angle_d_);
	bool Loop(resave_marker_pose::ResaveMarkerPose::Request &req,\
			  resave_marker_pose::ResaveMarkerPose::Response &res);
};

	bool Resave_Marker_Pose::selectMarkerPose(std::vector<marker_pose>& src_,\
						     std::vector<marker_pose>& dst_,\
						     double delta_threshold_,\
						     int n_d_,\
						     double delta_angle_threshold_,\
	...
	void Resave_Marker_Pose::getMarkerMsgCallback(const aruco_msgs::MarkerArray::ConstPtr &msg)
	{
		if(!start_move)
			return;
            ...
            */
			...
			if(pre_temp_marker_pose.size() < N_MP){
                ...
                */
				...
				tf::Transform mapTomarker(pre_q,pre_marker_position);
				...
				*/
				...
				//tf::Transform p = t;
				tf::Transform pre_p = mapTomarker;
				/*
                ...
                */
                ...
                if(!outfile.is_open())
                    ...
                    outfile << pre_p.getOrigin().x() << "," \
                            << pre_p.getOrigin().y() << ","\
                            << pre_p.getOrigin().z() << ","\
                            << pre_p.getRotation().getX() << ","\
                            << pre_p.getRotation().getY() << ","\
                            << pre_p.getRotation().getZ() << ","\
                            << pre_p.getRotation().getW() << std::endl;
	...
	bool Resave_Marker_Pose::Loop(resave_marker_pose::ResaveMarkerPose::Request &req,
			  				      resave_marker_pose::ResaveMarkerPose::Response &res){
...
{
    ros::init(argc, argv, "resave_marker_pose_node");
	ros::NodeHandle private_nh("~");
    
    std::string resave_marker_pose_path;
    private_nh.param<std::string>("resave_marker_pose_path",resave_marker_pose_path, "");
    ROS_INFO_STREAM("resave_marker_pose_path:" << resave_marker_pose_path);

	Resave_Marker_Pose obj(resave_marker_pose_path,private_nh);
	tf_listener = new tf::TransformListener(private_nh);
	tf_broadcaster = new tf::TransformBroadcaster();
