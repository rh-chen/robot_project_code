...
void load_image_points(int board_width, int board_height, float square_size, int num_imgs, 
  ...
  for (int i = 0; i <= num_imgs; i++) {
    ...
    std::cout << "img1.channel:" << img1.channels() << std::endl;
    std::cout << "img2.channel:" << img2.channels() << std::endl;
  ...
  CV_CALIB_CB_ADAPTIVE_THRESH | CV_CALIB_CB_FILTER_QUADS);
    ...
    if (found1)
    {
      cv::cornerSubPix(gray1, corners1, cv::Size(3, 3), cv::Size(-1, -1),
  cv::TermCriteria(CV_TERMCRIT_EPS | CV_TERMCRIT_ITER, 30, 0.1));
      //cv::drawChessboardCorners(gray1, board_size, corners1, found1);
    }
    if (found2)
    {
      cv::cornerSubPix(gray2, corners2, cv::Size(3, 3), cv::Size(-1, -1),
  cv::TermCriteria(CV_TERMCRIT_EPS | CV_TERMCRIT_ITER, 30, 0.1));
