...
#include <ram_path_planning/boustrophedon_explorator.h>
...
bool ZigZagCpp(ram_path_planning::Cpp::Request& req,
...
#if 1
	BoustrophedonExplorer BE_;

	std::vector<geometry_msgs::Pose2D> exploration_path;
	double map_resolution = req.map_resolution;
	cv::Point2d map_origin(req.map_origin_x, 
						   req.map_origin_y);
	cv::Point starting_position((req.start_position_x-map_origin.x)/map_resolution,
								(req.start_position_y-map_origin.y)/map_resolution);
	double grid_spacing_in_meter = req.deposited_material_width*std::sqrt(2);
	double grid_spacing_in_pixel = grid_spacing_in_meter/map_resolution;
	double path_eps = 3.0;
	Eigen::Matrix<float, 2, 1> zero_vector;
	zero_vector << 0,0;
	double min_cell_area_ = 0.6;

	BE_.getExplorationPath(bin, 
						   exploration_path,
						   map_resolution,
						   starting_position,
						   map_origin,
	...
	ROS_INFO("exploration_path_pose_size:%d",exploration_path.size());
#endif
#if 0
...
#endif
	std::vector<cv::Point2f> contour_pose;
	std::vector<cv::Point2f> com_contour_pose;
	double epsilion_approx = 0.2;
	for(int i = 0;i < exploration_path.size();i++){
		contour_pose.push_back(cv::Point2f(exploration_path[i].x,exploration_path[i].y));
	}

	cv::approxPolyDP(cv::Mat(contour_pose), com_contour_pose,epsilion_approx,false);
	
	geometry_msgs::PoseStamped cur_pose;
	for(int i = 0;i < com_contour_pose.size();i++){
		cur_pose.pose.position.x = com_contour_pose[i].x;
		cur_pose.pose.position.y = com_contour_pose[i].y;
		cur_pose.pose.position.z = 0;
	      
		res.path.poses.push_back(cur_pose);
	}
	return true;
}
