...
TEST(TestSuite, selectPosesInEmptyTrajectory)
{
  ...
  bool success_1 = (get_trajectory_size_srv.response.trajectory_size != 0) ? true : false;
...
TEST(TestSuite, getTrajectorySize)
{
  ...
  bool response = (srv.response.trajectory_size == 6) ? true : false;
  EXPECT_TRUE(success);
...
TEST(TestSuite, numberOfLayersLevels)
{
  ...
  bool response = (srv.response.number_of_layers == 3) ? true : false;
...
TEST(TestSuite, layerSize)
{
  ...
  bool response = (srv.response.layer_size == 3) ? true : false;
