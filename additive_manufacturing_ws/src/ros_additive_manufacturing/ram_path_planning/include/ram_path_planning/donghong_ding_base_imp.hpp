...
template<class ActionSpec>
                                                              ...
                                                              ram_msgs::AdditiveManufacturingTrajectory &msg)
  {
    ...
    //send layers_vector
    //divideInLayersWithOnePolygon(layers_vector,msg,first_layer,slicing_direction);
    connectLayersWithOnePolygon(gh, current_progrress_value, next_progress_value, layers, msg, first_layer);
    return "";
  }

template<class ActionSpec>
  void DonghongDingBase<ActionSpec>::connectYamlLayers(actionlib::ServerGoalHandle<ActionSpec> &gh,
                                                       const int current_progrress_value,
                                                       const int next_progress_value,
