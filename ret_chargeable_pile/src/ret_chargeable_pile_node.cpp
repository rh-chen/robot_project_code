...
double robot_to_marker_distance = 0.65;
...
class robot_control{
	...
	{
        mtx4.lock();
		ROS_INFO("msg->header.frame_id:%s\n",msg->header.frame_id.c_str());
#if 1
			...
			if(is_position_unsuitable)
			{
				if(forward_to_marker_nearby)
				{
					if(alter_marker_y > position_threshold)
					{
						...
						double angle_to_nearby = atan(fabs((alter_marker_z-robot_to_marker_distance)/alter_marker_y));
					...
					else if(alter_marker_y < -position_threshold)
					{
                        ...
                        straight_n_distance(fabs(alter_marker_z - robot_to_marker_distance),false);
					}
				}
				else
				{
