...
namespace line_detection_and_rotation {
	...
	bool lineLsdTransform(cv::Mat& bin_,double& angle)
	{
        ...
        for(int j = 0;j < line_num;j++){
            ...
            if(std::sqrt(delta_x*delta_x+delta_y*delta_y) > std::sqrt(21*21*2)){
                //line_angle = cv::fastAtan2(delta_y,delta_x);
                double current_direction = std::atan2(delta_y, delta_x);
                while (current_direction < 0.0)
                    current_direction += CV_PI;

                while (current_direction > CV_PI)
                    current_direction -= CV_PI;

                direction_histogram.addData(current_direction, std::sqrt(delta_y*delta_y+delta_x*delta_x));
            }
	    }

        free_ntuple_list(ntl);
        
        angle = direction_histogram.getMaxBinPreciseVal()*180/CV_PI;
