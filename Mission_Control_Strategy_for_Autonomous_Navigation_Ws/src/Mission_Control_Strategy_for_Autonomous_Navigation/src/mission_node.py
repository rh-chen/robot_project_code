...
class MissionHandler:
    ...
    def updateCurrentWaypoint(self):
        ...
        for i in range(self.frontiers.shape[0]):
            if np.abs(self.current_wp.x - self.frontiers[i, 0]) < 0.1 and \
                    np.abs(self.current_wp.y - self.frontiers[i, 1]) < 0.1:
    ...
    def proposeWaypoints(self):
        ...
        waypoints_tried = 0
        rospy.loginfo("frontiers_size:%d",self.frontiers.shape[0])
