...
PotentialMapMaker::PotentialMapMaker(){
    ...
    projected_map_sub = node_handle.subscribe("/map", 10, &PotentialMapMaker::projectedMapCallback, this);
    potential_map_pub = node_handle.advertise<autonomous_nav::PotentialGrid>("/potential_map", 10);
                                   ...
                                   autonomous_nav::PotentialPlanner::Response& response)
{
    ...
    while(current_cell.x != robot_cell.x || current_cell.y != robot_cell.y){
        ...
        int i_end = std::min(current_cell.x+2, (int)last_potential_map.info.width-1);

        int j_start = std::max(0, current_cell.y-1);
        int j_end = std::min(current_cell.y+2, (int)last_potential_map.info.height-1);
    ...
    std::cout << __FILE__ << __LINE__ << std::endl;
    std::cout << "##############################pixel_path_size:" << pixel_path.size() << std::endl;
