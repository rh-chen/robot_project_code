...
#include "additive_cpp/histogram.h"
#include "lsd.h"
...
namespace line_detection_and_rotation {
	...
	bool lineLsdTransform(cv::Mat& bin_,double& angle)
	{
		...
		double line_length_threshold = 30.0;
        int line_num = ntl->size;

		Histogram<double> direction_histogram(0, 2*CV_PI, 2*36);
        ...
        for(int j = 0;j < line_num;j++){
            ...
            if(sqrt(delta_x*delta_x+delta_y*delta_y) > line_length_threshold){
                line_angle = cv::fastAtan2(delta_y,delta_x);
                line_length = sqrt(delta_x*delta_x+delta_y*delta_y);
			
				direction_histogram.addData(line_angle,line_length);
            }
	    }

        angle = direction_histogram.getMaxBinPreciseVal();
	...
	bool MapRotate(additive_cpp::MapRotate::Request &req,     
    			   additive_cpp::MapRotate::Response &res) {
		...
		if(!map.empty()){
				...
				*/
				...
				bool res_line = lineLsdTransform(map,nAngle);
