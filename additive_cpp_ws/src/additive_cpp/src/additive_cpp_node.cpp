...
#include "additive_cpp/ModifyMap.h"
#include "additive_cpp/MapRotate.h"
...
public:
	...
	void clickCallBack(const geometry_msgs::PointStamped::ConstPtr &msg)
	{
            ...
            ros::ServiceClient map_modify_client = n.serviceClient<additive_cpp::ModifyMap>("/sweeper/map_modify_srv");
			ros::ServiceClient map_rotate_client = n.serviceClient<additive_cpp::MapRotate>("/sweeper/map_rotate_srv");
			ros::ServiceClient mapClient = n.serviceClient<nav_msgs::GetMap>("/static_map");

			additive_cpp::MapRotate map_rotate_srv;


            additive_cpp::ModifyMap map_modify_srv;
			...
			if (res_map_rotate) {
					...
					//start.pose.position.x = msg->point.x;
					...
					if(true){
							ROS_INFO("call additive_cpp service");
							...
							ROS_ERROR("Failed to call service /sweeper/additive_cpp_srv");
			} else {
