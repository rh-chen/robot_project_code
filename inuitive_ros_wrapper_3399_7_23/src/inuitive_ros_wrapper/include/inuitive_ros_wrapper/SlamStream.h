...
namespace InuDev 
{
    ...
    {
        // \brief    translation angles in Radians
		float mTranslation3D[3];

        // \brief    Rotation angles in Radians
        float mQuaternion[4];

		float mTransformMatrix[TRANSFORM_MATRIX_SIZE];
    };
    ...
    public:
        ...
        typedef std::function<void(std::shared_ptr<CSlamStream>, const CSlamFrame&,  CInuError)> CallbackFunction;
        ...
        virtual CInuError GetFrame(CSlamFrame& oSlamFrame, unsigned int iTimeout = FPS_BASED_TIMEOUT) = 0;
