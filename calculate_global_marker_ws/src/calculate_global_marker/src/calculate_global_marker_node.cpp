...
#include <calculate_global_marker/GetMarkerGlobalPose.h>
...
class calculate_global_pose{
	...
	bool selectMarkerPose(std::vector<marker_pose>& src_,\
			      ...
			      int n_angle_d_);
	bool Loop(calculate_global_marker::GetMarkerGlobalPose::Request &req,\
			  calculate_global_marker::GetMarkerGlobalPose::Response &res);
};
	...
	void calculate_global_pose::getMarkerMsgCallback(const aruco_msgs::MarkerArray::ConstPtr &msg)
	{
		...
		if((msg->header.frame_id == markerarray_msg_frame_id) && (msg->markers.size() > 0)){
			...
			if(temp_marker_pose.size() < N_MP){
				...
				bool sel_res = selectMarkerPose(temp_marker_pose,sel_marker_pose,0.08,3,0.15,1);
	...
	bool calculate_global_pose::Loop(calculate_global_marker::GetMarkerGlobalPose::Request &req,
			  						 calculate_global_marker::GetMarkerGlobalPose::Response &res){
		...
		if(req.start){
			...
			*/
			...
			while(ros::ok()){
				...
				if(markerVisible){
					if(enoughMarker){
                        if(std::isnan(goal.pose.position.x) || std::isnan(goal.pose.position.y) || std::isnan(goal.pose.position.z)){
                            res.status = 1;
					        res.text.data = "fail";
					        res.goal = geometry_msgs::PoseStamped();
					        res.pre_goal = geometry_msgs::PoseStamped();
