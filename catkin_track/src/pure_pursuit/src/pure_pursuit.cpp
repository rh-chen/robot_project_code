                             ...
                             lookahead_frame_id_("lookahead")
{
  ...
  nh_private_.param<double>("lookahead_distance", ld_, 0.2); //0.5
  ...
  try
  {
    ...
    for (; idx_ < path_.poses.size(); idx_++)
    {
      ...
      if (distance(path_.poses[idx_].pose.position, tf.transform.translation) > ld_)
      {
        ...
        ROS_INFO_STREAM("====  path_.poses[idx_].pose.position" << path_.poses[idx_].pose.position << "    tf.transform.translation " << tf.transform.translation );
        ...
        ROS_INFO_STREAM("==== nextPoint. x:" << path_.poses[idx_].pose.position.x << "  y:" << path_.poses[idx_].pose.position.y);
        ROS_INFO_STREAM("==== currentPoseTf. x:" << tf.transform.translation.x  << "  y:" << tf.transform.translation.y );
    ...
    {
      //ROS_INFO_STREAM("====  idx_:" << idx_ << "  path_.poses.size():" << path_.poses.size());
      ...
      //ROS_INFO_STREAM("====  fabs(F_bl_end.p.x()):" << fabs(F_bl_end.p.x()) << "  pos_tol_:" << pos_tol_);
    ...
    if (!goal_reached_)
    {
      ...
      cmd_vel_.angular.z = std::max(cmd_vel_.angular.z, -w_max_);

      ROS_INFO_STREAM("==== z:" << cmd_vel_.angular.z << "  min(" << (2*v_ / ld_2 * yt )<<  "," << w_max_ <<  ")" << "  v_:" << v_ << "  ld_2:" << ld_2 << "  yt:" << yt );
      ...
      if(cmd_vel_.angular.z > 0.5){
          cmd_vel_.linear.x = (cmd_vel_.linear.x / cmd_vel_.angular.z) * 0.5;  //if z > 0.5, slowly x, so faster z
      }
