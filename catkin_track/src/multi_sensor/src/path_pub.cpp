...
main (int argc, char **argv)
{
    ...
    ros::Rate loop_rate(1);
        ...
        pose1.pose.position.x = 0.2;
        pose6.pose.position.y = -0.6;
        pose6.pose.orientation = goal_quat;
        pose6.header.stamp=current_time;
        pose6.header.frame_id="map";

        pose7.pose.position.x = 0.0;
        pose7.pose.position.y = -0.6;
        pose7.pose.orientation = goal_quat;
        pose7.header.stamp=current_time;
        pose7.header.frame_id="map";

        pose8.pose.position.x = 0.0;
        pose8.pose.position.y = -0.8;
        pose8.pose.orientation = goal_quat;
        pose8.header.stamp=current_time;
        pose8.header.frame_id="map";

        pose1.pose.position.x = 0.2;
        pose9.pose.position.y = -0.8;
        pose9.pose.orientation = goal_quat;
        pose9.header.stamp=current_time;
        pose9.header.frame_id="map";



        //pose10.pose.position.x = 2;
        //pose10.pose.position.y = 0;
        //pose10.pose.orientation = goal_quat;
        //pose10.header.stamp=current_time;
        ...
        path.poses.push_back(pose8);
        path.poses.push_back(pose9);
        //path.poses.push_back(pose10);
