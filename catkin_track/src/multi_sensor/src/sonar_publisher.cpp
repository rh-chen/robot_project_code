...
int main(int argc, char**argv)
{
    ...
    ros::init(argc, argv, "range_msg");
        ...
        ros::Publisher sonar0_pub = nh.advertise<sensor_msgs::Range>("right_sonar_link", 5);
        ros::Publisher sonar1_pub = nh.advertise<sensor_msgs::Range>("sonar", 5);
        ...
        while (ros::ok())
        {
            ...
            if( (rcv_buf[0] & 0XFF) == 0XCA && (rcv_buf[1] & 0XFF) == 0X04 && (rcv_buf[7] & 0XFF) == 0XFF )
            {
                char checkCode = rcv_buf[2] ^ rcv_buf[3] ^rcv_buf[4] ^rcv_buf[5];
                if(checkCode == rcv_buf[6]){
                double distance = (double)(rcv_buf[5]&0xff);
                    ...
                    rangeMsg1.header.frame_id = "/sonar";
                    rangeMsg1.radiation_type = 0,                     //0=ultrasonic, 1=IR
                    rangeMsg1.field_of_view = 0.05;
                    rangeMsg1.min_range = 0.0;
                    rangeMsg1.max_range = 2.0;



                    switch(rcv_buf[3]){
                        case 0x00:
                            rangeMsg0.range = distance / 100;
                            sonar0_pub.publish(rangeMsg0);
                            printf("ultra 0's distance is %d \n",distance);
                            break;
                        case 0x01:
                            rangeMsg1.range = distance / 100;
                            printf("ultra 1's distance is %d \n",distance);
