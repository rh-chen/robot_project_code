...
namespace Cpp
{
  ...
  DonghongDingBase::DonghongDingBase(const std::string name,
  ...
  DonghongDingBase::~DonghongDingBase()
  {
  }

// Connect layers
  std::string DonghongDingBase::connectMeshLayers(const int current_progrress_value,
												 ...
												 )
  {
    ...
    //send layers_vector
    //divideInLayersWithOnePolygon(layers_vector,msg,first_layer,slicing_direction);
    connectLayersWithOnePolygon(current_progrress_value, next_progress_value,layers,msg,first_layer);
    return "";
  }

  void DonghongDingBase::connectYamlLayers(
      ...
      for (auto polygon : polygon_vector)
      {
        ...
        for (vtkIdType layer_id(0); layer_id < number_of_layers; ++layer_id)
        {
          ...
          for (vtkIdType point_id(0); point_id <= n_points; ++point_id)
          {
            ...
            ram_path_planning::AdditiveManufacturingPose pose_srv;
  ...
  void DonghongDingBase::connectLayersWithOnePolygon(
                                                     const int current_progrress_value,
                                                     const int next_progress_value,
                                                     std::vector<Layer> &layers,
													 ram_path_planning::AdditiveManufacturingTrajectory &msg,
                                                     const unsigned first_layer)
  {
    ...
    for (unsigned layer_id(0); layer_id < n_layers; ++layer_id)
    {
      ...
      ram_path_planning::AdditiveManufacturingPose first_pose_srv;
...
// Others functions
  double DonghongDingBase::angleBetweenVectors(const double v1[3],
                                               const double v2[3])
  {
  ...
  void DonghongDingBase::computeNormal(vtkPoints *p,
                                       double *n)
  {
  ...
  bool DonghongDingBase::offsetPolygonContour(const Polygon poly_data,
  ...
  bool DonghongDingBase::intersectionBetweenContours(const Polygon poly_data)
  {
  ...
  void DonghongDingBase::identifyRelationships(const Polygon poly_data,
  ...
  bool DonghongDingBase::organizePolygonContoursInLayer(const Polygon poly_data,
  ...
  bool DonghongDingBase::mergeColinearEdges(const Polygon polygon,
                                            const double tolerance)
  {
    vtkSmartPointer<vtkCellArray> polygon_array = vtkSmartPointer<vtkCellArray>::New();
    vtkSmartPointer<vtkPoints> points = vtkSmartPointer<vtkPoints>::New();
...
/*  void DonghongDingBase::divideInLayersWithOnePolygon(std::vector<Layer> &layers,
  													  additive_cpp::AdditiveManufacturingTrajectory &msg,
                                                      const unsigned first_layer)
  {
    std::vector<Layer> layers_vector;
    Layer current_layer;
